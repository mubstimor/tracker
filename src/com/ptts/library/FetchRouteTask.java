...
public class FetchRouteTask extends AsyncTask<String, Void, JSONArray> {

    public RouteListScreen listScreen = null;
    ...
    protected void onPostExecute(JSONArray todoJsonArray) {
        ...
        for (int i = 0; i < todoJsonArray.length(); i++) {
            try {
                ...
                routeItems.add(new RouteItem(jsonObject.getString("title")));
                Log.i("FETCHING DATA", jsonObject.getString("title"));
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
